...
export const videoRouter = router({
  ...
  create: protectedProcedure.input(z.object({ title: z.string().trim().min(2).max(255), outputFormat: z.enum(["vertical_9_16", "landscape_16_9", "square_1_1"]), targetDurationSeconds: z.number().int().min(5).max(3600), contentProjectId: z.number().int().positive().optional(), sourceAssetId: z.number().int().positive().optional(), thumbnailAssetId: z.number().int().positive().optional(), outputAssetId: z.number().int().positive().optional(), storageMetadata: z.object({ attachmentMode: z.literal("owner_scoped_managed_asset").optional(), sourceLabel: z.string().trim().min(1).max(160).optional(), notes: z.string().trim().max(1000).optional() }).strict().optional(), editPlan: z.object({ clipping: z.boolean(), silenceRemoval: z.boolean(), captions: z.boolean(), voiceOver: z.boolean(), subtitles: z.boolean(), aspectRatioConversion: z.boolean(), execution: z.literal("external_render_required") }).optional() })).mutation(async ({ ctx, input }) => {
