...
## 2026-08-21 — Synchronized resilience-control validation

The private command-center resilience update was synchronized after private `main` advanced during the continuation audit. The missing non-secret credential-management link registry was restored with official provider settings URLs, and the GitHub test suite was made hermetic: live credential validation is opt-in, while mocked review retrieval supplies a test-only token and retains the production credential gate. No real GitHub token was read, printed, or persisted.

The stale Vite JSX locator plugin was removed because its package was absent from the synchronized dependency graph and it prevented TypeScript resolution. The deprecated `pnpm` manifest settings were migrated into `pnpm-workspace.yaml`, preserving the Wouter patch and Tailwind nanoid override without the former configuration warning. Frozen installation, TypeScript checking, and the full Vitest suite passed with 83 passing tests and one intentionally skipped opt-in live credential check. A production build completed, and a localhost-only production server returned the application shell before it was stopped.

The synchronized manifest defines the Vite 7 toolchain, and its lockfile was regenerated from that manifest after a stale Vite 8 lock contract was detected. Frozen installation, TypeScript, and the full test suite passed again. The first production-build retry was terminated under transient sandbox memory pressure; after confirmed leftover local runtime probes were stopped and memory was released, the Vite 7 build completed and a localhost-only production probe returned the application shell before its process was stopped. The new Drizzle migration files were inspected as source contracts only; no managed database migration was executed from this sandbox.
