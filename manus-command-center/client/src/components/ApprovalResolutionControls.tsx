...
export function AgentDispatchControl({ agentId, agentName }: { agentId: number; agentName: string }) {
  ...
  const dispatches = trpc.agents.dispatches.useQuery({ agentId });
  const pendingDispatch = dispatches.data?.find(dispatch => dispatch.status === "needs_approval");
  const request = trpc.agents.requestDispatch.useMutation({ onSuccess: async result => { setStatus(result.status); await Promise.all([utils.commandCenter.logs.invalidate(), utils.agents.dispatches.invalidate({ agentId })]); toast.message(result.status === "needs_approval" ? "Dispatch recorded for review." : "Dispatch was recorded as queued; no external action was invoked."); }, onError: error => toast.error(error.message) });
  const resolve = trpc.agents.resolveDispatch.useMutation({ onSuccess: async result => { setStatus(result.status); await Promise.all([utils.commandCenter.logs.invalidate(), utils.agents.dispatches.invalidate({ agentId })]); toast.success(result.status === "approved" ? "Dispatch approval recorded. No external action was invoked." : "Dispatch denial recorded."); }, onError: error => toast.error(error.message) });
  const busy = request.isPending || resolve.isPending;
  const visibleStatus = pendingDispatch ? "needs_approval" : status;
  return <Card className="border border-cyan-100 bg-cyan-50/40 shadow-none"><CardContent className="space-y-3 p-4"><div><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-cyan-800">Governed dispatch</p><p className="mt-1 text-xs text-slate-600">Record a scoped task for <span className="font-semibold">{agentName}</span>. Approval never invokes an external action.</p></div><div className="grid gap-2 sm:grid-cols-[1fr_150px_auto]"><div className="space-y-1"><Label className="sr-only" htmlFor={`dispatch-task-${agentId}`}>Task</Label><Input id={`dispatch-task-${agentId}`} value={task} onChange={event => setTask(event.target.value)} placeholder="Describe the intended task" /></div><Select value={action} onValueChange={value => setAction(value as typeof action)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{dispatchActions.map(item => <SelectItem key={item} value={item}>{item.replace("_", " ")}</SelectItem>)}</SelectContent></Select><Button size="sm" disabled={busy || task.trim().length < 3} onClick={() => request.mutate({ agentId, task, action })}><Send className="mr-1.5 size-3.5" />Request</Button></div>{visibleStatus === "needs_approval" ? <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3"><span className="text-xs text-amber-950">Requested · approval is required before any authorized adapter could continue.</span><div className="flex gap-2"><Button size="sm" disabled={busy} onClick={() => pendingDispatch && resolve.mutate({ dispatchId: pendingDispatch.id, decision: "approved" })}><Check className="mr-1 size-3.5" />Approve</Button><Button size="sm" variant="outline" disabled={busy} className="border-rose-200 text-rose-700" onClick={() => pendingDispatch && resolve.mutate({ dispatchId: pendingDispatch.id, decision: "denied" })}><X className="mr-1 size-3.5" />Deny</Button></div></div> : status !== "idle" ? <p className="font-mono text-[10px] uppercase tracking-[0.1em] text-slate-500">{status.replace("_", " ")} · audit event recorded</p> : null}</CardContent></Card>;
}
