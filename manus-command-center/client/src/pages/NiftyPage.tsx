...
export default function NiftyPage() {
  ...
  return <div className="mx-auto max-w-[1250px] space-y-6">
    ...
    {nifty.isLoading ? <div className="blueprint-card p-16 text-center text-sm text-slate-500">Loading NIFTY market data…</div> : nifty.isError ? <div className="blueprint-card p-16 text-center"><p className="text-sm text-rose-700">Market data could not be loaded. This does not affect saved informational watches or alerts.</p><Button className="mt-4" variant="outline" onClick={() => nifty.refetch()}>Retry market data</Button></div> : data ? <><section className="grid gap-4 md:grid-cols-3"><Card className="blueprint-card border-0"><CardContent className="p-5"><p className="eyebrow">LAST AVAILABLE</p><p className="mt-3 text-3xl font-black">{data.latest?.toLocaleString("en-IN", { maximumFractionDigits: 2 }) ?? "—"}</p><p className="mt-2 text-xs text-slate-500">{data.currency} · {data.name}</p></CardContent></Card><Card className="blueprint-card border-0"><CardContent className="p-5"><p className="eyebrow">SESSION CHANGE</p><p className={`mt-3 text-3xl font-black ${(data.changePercent ?? 0) >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{data.changePercent === null ? "—" : `${data.changePercent >= 0 ? "+" : ""}${data.changePercent.toFixed(2)}%`}</p><p className="mt-2 text-xs text-slate-500">Absolute change: {data.change?.toFixed(2) ?? "—"}</p></CardContent></Card><Card className="blueprint-card border-0"><CardContent className="p-5"><p className="eyebrow">DATA STATUS</p><Badge variant="outline" className="mt-4 font-mono text-[10px]">{data.dataStatus}</Badge><p className="mt-3 text-xs text-slate-500">{data.source}<br />Retrieved {new Date(data.retrievedAt).toLocaleString()}</p></CardContent></Card></section><Card className="blueprint-card border-0"><CardContent className="p-6"><p className="font-semibold">Three-month close history</p><div className="mt-5 h-80"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.points}><defs><linearGradient id="niftyFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#06b6d4" stopOpacity={0.28}/><stop offset="100%" stopColor="#06b6d4" stopOpacity={0}/></linearGradient></defs><XAxis dataKey="timestamp" tickFormatter={value => new Date(value).toLocaleDateString("en-IN", { month: "short", day: "numeric" })} tick={{ fontSize: 11 }} /><YAxis dataKey="close" domain={["dataMin - 100", "dataMax + 100"]} tick={{ fontSize: 11 }} /><Tooltip labelFormatter={value => new Date(Number(value)).toLocaleDateString("en-IN")} formatter={(value: number) => value.toLocaleString("en-IN", { maximumFractionDigits: 2 })} /><Area type="monotone" dataKey="close" stroke="#0891b2" strokeWidth={2} fill="url(#niftyFill)" /></AreaChart></ResponsiveContainer></div></CardContent></Card></> : <div className="blueprint-card p-16 text-center text-sm text-rose-700">Market data is temporarily unavailable. Please retry later.</div>}
  </div>;
}
